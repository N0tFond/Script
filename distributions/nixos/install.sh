...
main() {
    ...
    if confirm "Show development environment setup for NixOS?" "y"; then
        ...
        echo "  • Add packages to configuration.nix or home.nix"
        echo "  • Use nix-shell for temporary environments"
        echo "  • Use direnv + shell.nix for project-specific environments"
        echo "  • Consider using flakes for reproducible development"
    fi
    
    # System cleanup
    cleanup_system
    
    success "🎉 NixOS configuration completed!"
    echo
    info "Next steps:"
